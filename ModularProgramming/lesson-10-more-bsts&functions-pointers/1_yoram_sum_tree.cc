...
bool sum_left_eq_sum_right(const struct Node* root)
{
    if (root == nullptr)
    {
        return true;
    ...
    suml = sum_tree(root->left);
    sumr = sum_tree(root->right);
    if (suml > sumr)
    return false;

    ok = sum_left_eq_sum_right(root->left);
    if (!ok)
    return false;

    ok = sum_left_eq_sum_right(root->right);
    return ok;
}
